...
1. Wählen Sie mit einer der folgenden Optionen den Dienst aus, für den Sie An- bzw. Abmeldungen vornehmen möchten:
   ...
   * **[!UICONTROL Von eingehender Transition]**: Verwenden Sie den in der eingehenden Transition angegebenen Dienst. Sie können beispielsweise eine Datei importieren, in der der zu verwaltende Dienst für jede Zeile spezifiziert wird. Der Dienst, für den der Vorgang ausgeführt werden soll, wird dann für jedes Profil dynamisch ausgewählt.

   ![Screenshot mit der Benutzeroberfläche für den Abonnementdienst des Workflows.](../assets/workflow-subscription-service.png)

1. Wählen Sie den auszuführenden Vorgang aus: **Anmelden** oder **Abmelden**.

   Wenn der Dienst in der eingehenden Transition definiert ist, legen Sie fest, wie dieser Vorgang abgerufen werden soll:
   ...
   * **Aktionstyp aus einem Pfad der eingehenden Transition auswählen**: Wählen Sie die Spalte der eingehenden Daten aus, in der für jeden Eintrag der auszuführende Vorgang angegeben ist. Sie können beispielsweise eine Datei importieren, in der der Vorgang festgelegt ist, der für jede Zeile in einer Spalte „Vorgang“ durchzuführen ist. 
     ...
     >[!CAUTION]
     >
     >Wenn Sie diese Option auswählen, wird bei der Aktivität **Abonnements** standardmäßig davon ausgegangen, über eine Link-Definition für die Tabelle **Dienste (nms)** zu verfügen, die im Workflow eingerichtet wurde. Dazu müssen Sie sicherstellen, dass Sie höher im Workflow eine Abstimmrelation in einer **Aktivität „Anreicherung“** konfiguriert haben. Ein Beispiel für die Verwendung dieser Option ist [hier](#uc2) verfügbar.

   ![Screenshot mit der Benutzeroberfläche für den Abonnementdienst des Workflows – Eingehende Transitionen.](../assets/workflow-subscription-service-inbound.png)

1. Um Empfängerinnen und Empfänger von ihrer An- bzw. Abmeldung bei dem ausgewählten Dienst in Kenntnis zu setzen, aktivieren Sie die Option **[!UICONTROL Bestätigungsnachricht senden]**. Der Inhalt dieser Benachrichtigung wird in einer Versandvorlage für den Informationsdienst definiert.
...
![Screenshot des Workflows zum Anmelden einer Zielgruppe für einen bestimmten Dienst](../assets/workflow-subscription-service-uc1.png)
...
![Screenshot des Workflows zum Aktualisieren mehrerer Abonnementstatus über eine Datei](../assets/workflow-subscription-service-uc2.png)
...
* Mit der Aktivität **[!UICONTROL Abstimmung]** werden die Daten der Datei als der Profildimension der Adobe Campaign-Datenbank zugehörig identifiziert. Das Feld **E-Mail** der Datei wird dem Feld **E-Mail** der Profil-Ressource zugeordnet.
  ...
  ![Screenshot des Workflows – Benutzeroberfläche zur Abstimmung von Anmeldediensten](../assets/workflow-subscription-service-uc2-reconciliation.png)

* Die Aktivität **[!UICONTROL Anreicherung]** erstellt eine Abstimmrelation zur Tabelle „Dienste (nms)“ sowie einen einfachen Join zwischen der Spalte „Dienst“ der hochgeladenen Datei und dem Feld „Interner Name“ der Dienste in der Datenbank.

  ![Screenshot des Workflows – Benutzeroberfläche zur Anreicherung von Abonnementdiensten](../assets/workflow-subscription-service-uc2-enrichment.png)

* **[!UICONTROL Abonnements]** identifiziert die zu aktualisierenden Dienste, die von der Transition kommen.

  Der **[!UICONTROL Kampagnentyp]** wird über das **operation**-Feld der Datei identifiziert. Hier können nur die Felder „Boolean“ oder „Integer“ ausgewählt werden. Wenn die Spalte Ihrer Datei, die den auszuführenden Vorgang enthält, nicht in der Liste erscheint, vergewissern Sie sich, dass Sie das Spaltenformat in der Aktivität **[!UICONTROL Datei laden]** richtig eingerichtet haben, wie zuvor in diesem Beispiel erläutert.

  ![Screenshot des Workflows – Benutzeroberfläche zum Abonnieren von Anmeldediensten](../assets/workflow-subscription-service-uc2-subscription.png)